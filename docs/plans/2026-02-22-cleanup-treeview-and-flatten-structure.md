...
**Files:**
- Delete: `src/pluginMarketplace/pluginTreeProvider.ts`
...
```bash
rm src/pluginMarketplace/pluginTreeProvider.ts
```
...
**Files:**
- Modify: `src/pluginMarketplace/types.ts:67-160`
- Modify: `src/extension.ts:5,161,193,221,241,261,288`

**Step 1: 移除 PluginTreeItem 类定义**

编辑 `src/pluginMarketplace/types.ts`，删除：
...
```bash
git add src/extension.ts src/pluginMarketplace/types.ts
...
**Files:**
- Modify: `tsconfig.json:15-18`
- Modify: `webview/tsconfig.json`

**Step 1: 更新扩展 tsconfig.json**

修改 `tsconfig.json`，移除 `webview` 路径别名（因为即将扁平化）：
...
**Step 1: 检查是否有 .gitignore**

```bash
ls -la .gitignore 2>/dev/null || echo "No .gitignore in "
```

**Step 2: 更新根目录 .gitignore**

确保 `.gitignore` 包含以下内容（移除 `` 前缀）：
...
git mv node_modules .
git mv package.json .
git mv tsconfig.json .
git mv jest.config.js .
```
...
# 检查并移动其他文件
ls -la 
git mv .eslintrc.json . 2>/dev/null || true
git mv .prettierrc . 2>/dev/null || true
```
...
```bash
git add -A
git commit -m "refactor: flatten directory structure - move * to root"
```
...
sed -i 's|webview/|webview/|g' README.md
```

**Step 2: 更新 TESTING.md**

```bash
sed -i 's|||g' TESTING.md
```

**Step 3: 更新 CLAUDE.md**

```bash
sed -i 's|src/|src/|g' CLAUDE.md
sed -i 's|webview/|webview/|g' CLAUDE.md
sed -i 's|out/|out/|g' CLAUDE.md
```

**Step 4: 更新 docs/plans 中的所有文档**

```bash
find docs/plans -name "*.md" -exec sed -i 's|||g' {} \;
```
